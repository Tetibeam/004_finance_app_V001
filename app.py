...
def dashboard():
    ...
    graphs["returns"] = pio.to_html(fig, full_html=False, include_plotlyjs='cdn')
    ...
    graphs["general_income_expenditure"] = pio.to_html(fig, full_html=False, include_plotlyjs='cdn')
    # 4. 一般収支
    fig = viz.display_general_balance(df_general)
    graphs["general_balance"] = pio.to_html(fig, full_html=False, include_plotlyjs='cdn')
    # 5. 特別収入・支出
    fig = viz.display_special_income_expenditure(df_special)
    graphs["special_income_expenditure"] = pio.to_html(fig, full_html=False, include_plotlyjs='cdn')
    # 6. 特別収支
    fig = viz.display_special_balance(df_special)
    #graphs["special_balance"] = fig.to_html(full_html=False)
    graphs["special_balance"] = pio.to_html(fig, full_html=False, include_plotlyjs='cdn')
